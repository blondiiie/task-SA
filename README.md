...
graph TD
    ...
    Broker{"2. Брокер сообщений(Kafka)"}
...
Допущения:
- Выбор брокера сообщений зависит от контекста. на данной схеме предпочтение отдалось Apache Kafka, так как это решение позволяет обеспечить высокую пропускную способность в пике нагрузки и гарантирует персистентность данных. В зависимости от количества пользователей и профиля нагрузки выбор решения может поменяться.
- Провайдеры доставки могут объединиться, используя FCM систему(или SaaS-сервис), поддерживающую как Android, так и IOS токены. Если критична безопасность и независимость от сторонних сервисов, то предлагается использовать прямую интеграцию(как на схеме)
